...
//UNUSED    private static Logger logger = Logger.getLogger(className);
            ...
            throws InvalidMidiDataException, IOException
    {
        ...
        for (int t = 1; t < midiTracks.length; t++)
        {
            ...
            Collection splitTracks = splitTrack(midiTrack);
            for (Iterator it = splitTracks.iterator(); it.hasNext();)
            {
    ...
    {
        Map channelToTrack=new HashMap();
        ...
        for(int i=0;i<count;i++)
        {
            ...
            if(status<0x00F0)
            {
                ...
                List thisTrack=(List) channelToTrack.get(new Integer(channel));
                if(thisTrack==null)
                {
                    thisTrack=new LinkedList();
    ...
    private static void encodeTrack(Song song, SongMeasure songMeasure,
            List singleTrack, int trackIndex)
    {
...
//                        	UNUSED                            r101 = value;
                ...
                else
                {
                            ...
                            || (command == ShortMessage.NOTE_OFF))
                    {
                        ...
                        {
                            SongEvent se = (SongEvent) openNotes
