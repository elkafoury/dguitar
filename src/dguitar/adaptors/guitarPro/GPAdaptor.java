...
public class GPAdaptor {
    ...
    private static RepeatedSongPhrase makeRepeatedSongPhrase(Song song,
            ListIterator itM, ListIterator itMTP, List tracks)
        ...
        for (int trackIndex = 1; trackIndex <= tracks.size(); trackIndex++) {
            ...
            for (Iterator itBeat = beats.iterator(); itBeat.hasNext();) {
                ...
                if (beat.isNoteBeat()) {
                    ...
                    List stringsPlayed = new LinkedList();
...
//						END
                        ...
                        stringObject = (Integer) itStrings.next();
     ...
     */
    private static SongPhrase phraseFactory(Song song, ListIterator itM,
            ListIterator itMTP, List tracks) throws GPFormatException {
        SongPhrase sp = null;

        GPMeasure measure = (GPMeasure) itM.next(); // peek inside the measure
    ...
    public static Song makeSong(GPSong piece) throws GPFormatException {
        ...
        int index = 1;
        for (Iterator it = tracks.iterator(); it.hasNext(); index++) {
